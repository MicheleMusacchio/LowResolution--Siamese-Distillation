...
if __name__ == '__main__':
    ...
    #_______________________STUDENT BASELINE_____________________________________________________________________________
    
    dataloader_train, dataloader_validation = Create_dataloader(Res_quality='LR',Batch_size=BATCH_SIZE,
    ...
    epoch_path=f"{MYPATH_STUDENT_BASELINE}/StudentBaseline_xx.pt"
    student.load_state_dict(torch.load(epoch_path))        
    '''
    
    optimizer = optim.Adam(student.parameters(), lr = 0.01) #optimizer = optim.SGD(model.parameters(), lr = 0.01)
    BCELOSS=nn.BCELoss().to(device)
    
    print('\n___________________________STUDENT BASELINE___________________________')
    print(f"\nTotal number of parameters: {sum(p.numel() for p in student.parameters())}")
    print(f"\nTotal number of trainable parameters: {sum(p.numel() for p in student.parameters() if p.requires_grad)}")
    
    
    model_name = 'StudentBaseline' 
    
    student_train_loss, student_train_acc, student_val_loss, student_val_acc =  train_val(model=student,
                                                                                          dataloader=dataloader_train, vald_loader=dataloader_validation,
                                                                                          n_epochs=N_EPOCH, start_epoch=START_EPOCH,
                                                                                          log_step = LOG_STEP, path_to_save_model=MYPATH_STUDENT_BASELINE)
    np.save(f"{MYPATH_STUDENT_BASELINE}/student_train_loss.npy",student_train_loss)
    np.save(f"{MYPATH_STUDENT_BASELINE}/student_train_acc.npy",student_train_acc)
    np.save(f"{MYPATH_STUDENT_BASELINE}/student_val_loss.npy",student_val_loss)
    np.save(f"{MYPATH_STUDENT_BASELINE}/student_val_acc.npy",student_val_acc)